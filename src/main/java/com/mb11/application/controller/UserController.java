...
public class UserController {
	...
	public com.mb11.application.payload.user.User getCurrentUser(@CurrentUser UserPrincipal userPrincipal) {
		...
		payloadUser.setCreatetime(LocalDate.parse(user.getCreatetime().toString()));
		payloadUser.setDob(LocalDate.parse(user.getDob().toString()));
		...
		payloadUser.setUpdatetime(LocalDate.parse(user.getUpdatetime().toString()));
